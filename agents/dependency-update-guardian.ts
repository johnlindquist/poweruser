...
function parseCliOptions(): ResolvedGuardianOptions | null {
  ...
  const autoUpdate = readBooleanFlag("auto-update", false);
  const securityOnly = readBooleanFlag("security-only", false);
  const createPR = readBooleanFlag("create-pr", false);
