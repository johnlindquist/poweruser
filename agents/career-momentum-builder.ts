...
const DEFAULT_OUTPUT_DIR = "./career-plan";
...
Options:
  ...
  --role <title>          Target role (e.g., "Senior Frontend Engineer")
  --resume <path>         Path to your resume file
  --output <path>         Output directory for action plan (default: ${DEFAULT_OUTPUT_DIR})
...
function parseOptions(): CareerBuilderOptions | null {
  const { values } = parsedArgs;
  const help = values.help === true || values.h === true;

  if (help || process.argv.slice(2).length === 0) {
    printHelp();
    return null;
  }

  // Validate that at least some input is provided
  const hasInput = values['job-url'] || values.github || values.role || values.resume;
  if (!hasInput) {
  ...
  const rawGithub = values.github;
  const rawRole = values.role;
  const rawResume = values.resume;
  ...
  const githubUsername = typeof rawGithub === 'string' && rawGithub.length > 0
    ? rawGithub
    : undefined;
  ...
  const resumePath = typeof rawResume === 'string' && rawResume.length > 0
    ? resolve(rawResume)
    : undefined;
...
if (options.githubUsername) console.log(`👤 GitHub: @${options.githubUsername}`);
if (options.jobUrl) console.log(`📄 Job Description: ${options.jobUrl}`);
if (options.resumePath) console.log(`📝 Resume: ${options.resumePath}`);
...
const defaultFlags: ClaudeFlags = {
  ...
  allowedTools: "Bash Read Write Grep Glob WebFetch WebSearch TodoWrite",
...
try {
  ...
  if (exitCode === 0) {
    ...
    console.log(`📂 Check ${options.outputDir}/ for your complete career action plan!`);
